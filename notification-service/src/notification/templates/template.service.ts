...
export class TemplateService {
   ...
   */
  getEmailSubject(type: NotificationType, data?: Record<string, unknown>): string {
  ...
  getEmailTemplate(type: NotificationType, data?: Record<string, unknown>): string {
    ...
    try {
      ...
      this.logger.error(`Failed to read email template ${metadata.fileName}: ${error}`);
      throw new NotFoundException(`Email template file not found: ${metadata.fileName}`);
    }
  }
   ...
   */
  getSmsTemplate(type: NotificationType, data?: Record<string, unknown>): string {
    const fileName = this.smsTemplateMap[type];
    if (!fileName) {
      throw new NotFoundException(`SMS template not found for type: ${type}`);
    }

    const templatePath = join(__dirname, '..', 'templates', 'sms', fileName);
